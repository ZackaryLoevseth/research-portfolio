...
- `sources/*.md`: original byte-for-byte source copies verified against the input SHA256 values before output creation. The fixture uses the exact version 0.3 of [the shared case](../shared-inquiry-case-01.md), pinned to its source commit in `case.json`. Use the pinned source URL to inspect the source version.
...
## Edition 2 reader path

Read the question, follow either proposal to its separate standing record, then follow the objection to the nonparticipation note. That note now makes the unassigned response responsibility and missing accessible route explicit. Previously those matters appeared only in the full source. This is a source-faithful correction of what the receiving note exposes, not a new response mechanism or evidence that anyone will respond. The same information can be kept in ordinary minutes.

The new source copy has project attribution and an updated exact hash; earlier source receipts remain historical. Current reproduction is recorded separately in VERIFICATION.md. No schema, authority semantics, license or historical model outcome changed.
